...
export function UnitDetailPanel({
  ...
  /** 按钮级权限：没有权限的动作按钮不渲染（DOM 里也不存在）；业务规则不允许时才保留可见但禁用 */
  canCreate: boolean;
  canUpdate: boolean;
  canDelete: boolean;
    ...
    <section className={styles.card} aria-label={`${node.name}详情`}>
      <div className={styles.head}>
        ...
        <div className={styles.actions}>
          ...
          {canUpdate && (
            <ActionButton disabled={false} title="" onClick={onEdit}>
              <EditOutlined /> 编辑
            </ActionButton>
          )}
          {canDelete && (
            <ActionButton disabled={blockedByReference} title={deleteReason} onClick={onDelete}>
              <DeleteOutlined /> 删除
            </ActionButton>
          )}
        </div>
              ...
              >
                <span className={styles.childMain}>
                  ...
                  <span className={styles.childMeta}>
                    ...
                    <span>·</span>
                    <span>{unitLevelLabel(child.level, levelDictionary)}</span>
                  </span>
                </span>
