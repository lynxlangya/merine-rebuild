 ...
 *
 * 与 {@code com.merine.rebuild.auth.AuthSessionRegressionSupport} 同一套做法（不换 Bean、
 * 不用 H2、不 mock Mapper；profile 只从环境变量取测试库连接并核对实际库名；每个测试方法
 * 先清理再插入自己命名空间的合成行；不用 {@code @Transactional} 包住测试，因为要跨多个
 * HTTP 请求观察会话失效与真实提交）。本模块另需一条配置，因此单独成类。
 *
 * 管理员角色编码用 {@link #ADMIN_ROLE_CODE} 覆盖 {@code merine.security.user-admin-role-codes}：
...
@TestPropertySource(properties =
        "merine.security.user-admin-role-codes=" + UserAdminRegressionSupport.ADMIN_ROLE_CODE)
abstract class UserAdminRegressionSupport {
    ...
    /**
     * 写请求：每次现取一个 CSRF 令牌，并按分派规则同时带上 Cookie 与请求头。
     * session 为 null 时不带会话，用来验证未登录时的拒绝。
     */
    ...
    /** 受保护接口 GET /api/auth/session：用来判定一个会话是否仍然有效。 */
    ...
    /** 字段级校验错误：400 VALIDATION_ERROR，且 fieldErrors 只落在预期字段上。 */
