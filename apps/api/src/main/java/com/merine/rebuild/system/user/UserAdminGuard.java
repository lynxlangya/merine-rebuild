...
public class UserAdminGuard {
    ...
    public UserAdminGuard(
            @Value("${merine.security.user-admin-role-codes:SYSTEM_ADMIN}") String configuredCodes) {
        this.adminRoleCodes = Arrays.stream(configuredCodes.split(","))
                .map(String::trim)
                .filter(code -> !code.isEmpty())
                .collect(Collectors.toUnmodifiableSet());
        if (adminRoleCodes.isEmpty()) {
            throw new IllegalStateException("merine.security.user-admin-role-codes 不能为空");
        }
