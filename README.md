...
> 当前进度：登录闭环与**用户管理**模块的前后端都已接真实后端与本地 MySQL。应用只保留首页、用户管理、工程诊断三个页面；尚未实现功能权限与数据范围模型，详见[本轮边界](#本轮边界)。设计稿只作样式参考，数据均为合成数据。
...
```bash
./scripts/dev.sh seed
# 需要换单位或角色时覆盖对应变量（默认是临湾海防支队 / 系统管理员，姓名为陈知远；均为虚构场景数据）
...
apps/web/             React、TypeScript、Vite、Ant Design、Router、TanStack Query
  ...
  src/features/       按用户用例拆分：auth、home、users、diagnostics
...
- `check`：建好测试库并跑 TypeScript、Node 内置前端回归测试、格式检查与 Maven 验证（含认证与用户管理回归测试）。
...
**已经接真实的**：登录、退出、当前身份与 CSRF（Spring Security Session + HttpOnly Cookie，账号落 `sys_user`，密码 BCrypt 哈希）；用户、单位与角色的查询与写入；路由守卫、身份恢复、会话失效处理。

**尚未实现**：功能权限与数据范围模型。当前的授权只有一道门——角色编码在 `merine.security.user-admin-role-codes`（默认 `SYSTEM_ADMIN`）里的账号才能调用用户管理接口；角色能使用什么功能、能看哪些业务数据仍未定义，数据范围与角色定义的后台也没有。
