...
- 字典用两张表：`sys_dict_type`（`dict_code` 全局唯一且**创建后不可改**、名称、说明、状态、version）与 `sys_dict_item`（`item_value` 在**同一类型内唯一**且不可改、标签、说明、排序、状态、version，外键指向类型）。取值与编码是业务数据里保存、代码里比较的稳定标识，改的只能是展示与状态；**类型与字典项都只停用、不删除**，因此不给级联删除留口子，也不提供删除接口——字典是全局参考数据，误删取值会让页面标签与选择项立刻退化，停用已经够表达"不再可选"。系统枚举（状态、菜单类型、单位层级）同样落这两张表，由迁移写入引导数据并只补缺失：不要为它们另开代码内真源，否则库里一套、代码一套，改哪边都不算数。
