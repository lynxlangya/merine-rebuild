...
| 索引/约束      | `idx_<表>_<用途>`、`uk_<表>_<用途>`、`ck_<表>_<规则>`                      | `uk_sys_user_login_name`                             |
...
| 状态/枚举 | 有长度上限的稳定字符串 code；Java 显式 code 映射，必要时 CHECK 或落字典表维护可选项；禁止 ordinal、显示中文或随重命名变化的 `name()` 作为持久契约            |
| JSON      | 仅用于边界明确的快照/扩展结构；定义字段、版本、大小与校验；需要关系约束、常用筛选/关联的字段独立建列                                                         |

- 表默认 InnoDB、utf8mb4，文本默认沿用 `utf8mb4_0900_ai_ci`；登录名、外部标识、业务唯一编码逐项定义大小写/重音/空白等价规则，必要时使用区分大小写的 collation。ASCII code 可用 `ascii_bin`；关联列的类型与字符集/collation 必须兼容，不能只让名字相同——没有外键以后，这份兼容性只能靠设计与实现保证。
...
- 引用字段写出目标表/字段及角色，并说明由谁保证不悬空（见第 5 节）。本项目不建物理外键，注释是关联语义的唯一书面来源。JSON 字段注明结构版本或就近结构说明的位置。
...
- **禁止物理外键与级联**：表结构不出现 `FOREIGN KEY`，也不出现 `ON DELETE/UPDATE CASCADE`。引用完整性由应用层保证：写路径在同一事务内先锁被引用行，再用**锁定读**校验——一致性读可能停在取锁之前的快照，会把「取锁前刚提交的删除」看漏，于是校验通过、写出的却是悬空引用。取锁顺序固定为授权锚点（内置角色行）→ 引用锁（`sys_unit` 目标行按主键 / `sys_menu` 全表）→ 关系表与权限码表；删除前校验引用（单位有下级或有用户、角色有成员一律拒绝）；引用目标用主键/唯一键，关联列有合适索引，注释写清目标与角色。引用锁按主键取：锁定读会同时锁二级索引记录与主键记录，而按主键删行是先主键后索引，两边顺序相反就会死锁。代价必须说清：失去数据库级兜底后，越过用例的写入（迁移、seed、手工 SQL、以后新增的旁路代码）不再被数据库挡住，靠巡检回归与结构断言补偿（见第 6 节）。历史迁移 V2/V7/V12/V13 的外键由 V17 统一移除，不回写旧迁移。
- 能由 NOT NULL、UNIQUE、CHECK 保证的不变量在数据库约束；涉及多行状态或跨表规则仍由受事务保护的用例维护，CHECK 不代替授权。
...
- 菜单资源树（`sys_menu`）把「结构」与「编码」分开约束：类型决定 `route_key` 与权限码是否必填（目录无码、页面必须有已注册的 `route_key`、页签/按钮必须有权限码），层级、成环与同级排序由受事务保护的用例校验；`sys_permission.permission_code` 是判权用的 authority，创建后不可修改、改名可以。停用只表达「不在导航与可分配项里」，不改变已授予的权限。
- 字典用两张表：`sys_dict_type`（`dict_code` 全局唯一且**创建后不可改**、名称、说明、状态、version）与 `sys_dict_item`（`item_value` 在**同一类型内唯一**且不可改、标签、说明、排序、状态、version，`dict_type_id` 关联类型）。取值与编码是业务数据里保存、代码里比较的稳定标识，改的只能是展示与状态；**类型与字典项都只停用、不删除**，因此不给级联删除留口子，也不提供删除接口——字典是全局参考数据，误删取值会让页面标签与选择项立刻退化，停用已经够表达"不再可选"。系统枚举（状态、菜单类型、单位层级）同样落这两张表，由迁移写入引导数据并只补缺失：不要为它们另开代码内真源，否则库里一套、代码一套，改哪边都不算数。
...
7. 新迁移不得出现 `FOREIGN KEY` 或级联；要表达关系时按第 5 节写注释、索引与引用锁。`SchemaForeignKeyRegressionTest` 同时断言实际库无外键、并对迁移文本做静态扫描，所以历史外键只在 V2/V7/V12/V13 保留原文（统一由 V17 移除），其余文件出现即失败。悬空引用由 `ReferenceIntegrityRegressionTest` 巡检。
